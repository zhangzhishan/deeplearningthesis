...
% 注意，没有其他单元连向偏置单元(即偏置单元没有输入)，因为它们总是输出 $ +1$。同时，我们用 $ s_l $表示第$  l$ 层的节点数（偏置单元不计在内）。}
...
\section{深度卷积神经网络}
% (Convolutional Neural Network, CNN) 其主要有以下几个特点：
% • 直接输入原始的图像数据
...
% 正如前一节所述，传统的神经网络模型中通常选择Sigmod或双曲正切作为激活函数。
% TODO: \textcolor{red}{介绍一下各个激活函数}
% 。然而，我们通常在卷积神经网络中使用以下ReLU激活函数。
...
使用二次函数来说明最小化，梯度下降的想法是跟随当前位置的负梯度，因为它描述了最陡的下降的方向。 学习率$ \ eta $描述在每个迭代步骤中采取的步长。 因此，梯度下降描述了一阶优化技术。

\begin{figure}[hbt]
	\centering
  % \includestandalone[width=\textwidth]{figures/networks/gradient-descent}
	\caption{梯度下降示意图}
